...
/**
 * @author Hasee  2021/3/15 21:23 周一
 * @version JDK 8.017
 * @description:
 */
@Service
public class SearchService {
    ...
    public SearchResponseVo search(SearchParamVo paramVo) {
        try {
            SearchResponse response = this.restHighLevelClient.search(new SearchRequest(new String[]{"goods"}, this.builderDsl(paramVo)), RequestOptions.DEFAULT);
...
//            Goods goods = new Goods();
            // 获取 hitsHit 对象中的 _source --> Json字符串
            String json = hitsHit.getSourceAsString();
            // 反序列化 json 字符串为 goods 对象
        ...
        // 解析 aggregations: 品牌 分类 规格参数的过滤条件
        ...
        if (!Collections.isEmpty(brandIdAggBuckets)){
            responseVo.setBrands(brandIdAggBuckets.stream().map(bucket ->{
                ...
                if (!Collections.isEmpty(nameAggBuckets)){
        ...
        if (!Collections.isEmpty(attrIdAggBuckets)){
            responseVo.setFilters(attrIdAggBuckets.stream().map(bucket ->{
                ...
                if (!Collections.isEmpty(valueAggBuckets)){
    ...
    private SearchSourceBuilder builderDsl(SearchParamVo paramVo) {
        ...
        if (priceFrom != null || priceTo != null) {
            ...
            if (priceFrom != null) {     // 如果 priceForm 不为空,添加大于等于
                rangeQuery.gte(priceFrom);
            }
            if (priceTo != null) {       // 如果 priceTo 不为空,添加小于等于
